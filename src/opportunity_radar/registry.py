...
DEFAULT_GREENHOUSE_HOSTS = ["job-boards.greenhouse.io", "boards.greenhouse.io"]
...
def common_crawl_indexes(config: dict[str, Any], *, fetcher: FetchFn = fetch_url) -> tuple[list[str], list[str]]:
    ...
    if isinstance(configured, list):
        indexes = [str(item).strip() for item in configured if str(item).strip()]
        if indexes:
            return indexes, []
    try:
        ...
        if isinstance(payload, list):
            indexes = [str(item.get("id", "")).strip() for item in payload if isinstance(item, dict) and str(item.get("id", "")).strip()]
            return indexes[:1], []
    except Exception as exc:
        return [], [f"common_crawl_index_list_failed:{type(exc).__name__}"]
    return [], ["common_crawl_index_list_empty"]


def common_crawl_cdx_url(index: str, host: str, limit: int) -> str:
    url_pattern = f"{host}/*/jobs/*"
    params = {
        "url": url_pattern,
        "output": "json",
    ...
    return f"https://index.commoncrawl.org/{quote(index, safe='')}/cdx?{query}"
        ...
        for index in indexes:
            for host in ats_hosts:
                ...
                try:
                    ...
                    errors.append(f"common_crawl_query_failed:{host}:{type(exc).__name__}")
            if remaining <= 0:
                break
...
def refresh_registry(
    ...
    *,
    ...
    state = store.load()
    discovery_result = discover_common_crawl_refs(root, config, fetcher=fetcher)
